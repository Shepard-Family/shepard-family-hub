...
function personRow(p){return `<div class="list-item"><div class="person-icon">${initials(p)}</div><div class="list-main"><strong>${esc(displayName(p))}</strong><small>${fmtDate(p.birthday,{month:'long',day:'numeric'})}${birthdayAgeTurning(p)!==null?` • Turning ${birthdayAgeTurning(p)}`:''} • ${esc(p.household_name||'Family')}</small></div></div>`}
...
function birthdayAgeTurning(person){
 if(!person?.birthday)return null;
 const birth=new Date(person.birthday+'T12:00');
 if(Number.isNaN(birth.getTime()))return null;
 const next=birthdayNext(person);
...
function birthdayCards(list){
 ...
 [...list].sort(birthdaySort).forEach(p=>months[new Date(p.birthday+'T12:00').getMonth()].push(p));
 return months.map((monthList,monthIndex)=>{
   ...
   return `<section class="section birthday-month"><div class="section-head"><div><h2>${monthName}</h2><p>${monthList.length} birthday${monthList.length===1?'':'s'}</p></div></div><div class="grid two">${monthList.map(p=>`<div class="card"><div class="list-item" style="padding:0;border:0"><div class="person-icon">${initials(p)}</div><div class="list-main"><strong>${esc(displayName(p))}</strong><small>${fmtDate(p.birthday,{month:'long',day:'numeric'})}${birthdayAgeTurning(p)!==null?` • Turning ${birthdayAgeTurning(p)}`:''} • ${esc(p.household_name||'Family')}</small></div></div></div>`).join('')}</div></section>`;
 }).join('');
}
